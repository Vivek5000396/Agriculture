...
def send_email(receiver_email, subject, content_type="verification", code=None):
    ...
    SENDER_EMAIL = "19102004vivek@gmail.com"
    SENDER_PASSWORD = "dxgogadmoshlkzvw"
...
if selected_page == "Project Landing Page":
        ...
        with st.form("land_area_form"):
            ...
            if calculate_area:
                if length > 0 and width > 0:
                    area = length * width
                    area_acres = area * 0.000247105  # Convert to acres
    ...
    with tab2:
        with st.form("water_requirement_form"):
            ...
            # Water requirements per acre for different crops (in liters)
            water_requirements = {
                "Rice": 5000000,
                "Wheat": 2000000,
                "Corn": 2500000,
...
# NEW IRRIGATION SCHEDULE SECTION
# NEW IRRIGATION SCHEDULE SECTION
elif selected_page == "Irrigation Schedule":
    st.markdown(
        """
    ...
    with st.form("irrigation_form"):
        ...
        crop_type = st.selectbox(
            "Select Crop Type",
            ["Rice", "Wheat", "Corn", "Cotton", "Sugarcane", "Vegetables", "Fruits", "Other"],
            help="Select the crop you want to plan irrigation for"
        )
        ...
        frequency = st.selectbox(
            ...
            help="How often the crop needs to be irrigated"
        )
        ...
        start_date = st.date_input(
            "Start Date",
            datetime.now(),
            help="Select when you want to begin the irrigation schedule"
        )
        ...
        cycles = st.slider(
            ...
            max_value=20,
            value=10,
            help="How many irrigation cycles to plan"
        )
        ...
        if submit_schedule:
            if not crop_type:
                ...
                # Download button
                csv = "\n".join([f"{item['Cycle']},{item['Date']},{item['Day']},{item['Crop']},{item['Notes']}" 
                               for item in schedule])
                st.download_button(
                    "Download Schedule",
                    data=f"Cycle,Date,Day,Crop,Notes\n{csv}",
                    file_name=f"{crop_type}_irrigation_schedule.csv",
                    mime="text/csv"
                )
    ...
    with st.form("yield_form"):
        ...
        land_size = st.number_input(
            ...
            value=1.0,
            step=0.1,
            help="Enter the size of your cultivated land"
        )
        
        # Soil quality selector
        soil_quality = st.select_slider(
            "Soil Quality",
            options=["Poor", "Average", "Good", "Excellent"],
            value="Average",
            help="Rate your soil quality"
        )
        
        # Irrigation type selector
        irrigation_type = st.radio(
            "Irrigation Method",
            ["Rain-fed", "Flood", "Drip", "Sprinkler"],
        ...
        if submit_yield:
            if not crop_type or land_size <= 0:
                ...
                # Yield estimates in kg/acre (approximate values)
                yield_data = {
    ...
    with st.form("weather_form"):
        ...
        with col1:
            city = st.text_input(
                ...
                help="Enter the city you want weather information for"
            )
        with col2:
            state = st.text_input(
                "State (Optional)",
                placeholder="e.g., Telangana, Maharashtra...",
                help="Enter the state for more precise location"
            )
            ...
            if not city:
                ...
                with st.spinner("Fetching weather data..."):
                    try:
                        ...
                        # Construct query (city alone or city,state)
                        ...
                        if 'error' in response:
                            ...
                            # Display location with state if available
    ...
    with st.form("price_prediction_form"):
        ...
        with col1:
            state = st.selectbox(
                ...
                ["Andhra Pradesh", "Telangana", "Karnataka", "Tamil Nadu", 
                 "Maharashtra", "Uttar Pradesh", "Punjab", "Other"],
                index=0,
                help="Select your state for localized price data"
            )
        with col2:
            crop_type = st.selectbox(
                "Select Crop Type",
                ...
                index=0,
                help="Select the crop you want price analysis for"
            )
        ...
        analysis_period = st.selectbox(
            ...
            index=1,
            help="Select time period for price trend analysis"
        )
        ...
        if get_prediction:
            with st.spinner("Analyzing market trends..."):
                try:
                    # Simulated price data (in a real app, you would call an API)
                    price_trends = {
                    ...
                    # Get trends for selected crop or default values
                    ...
                    # Show line chart of price trends
                    st.line_chart(
                        data=trends,
                    ...
                    # AI price prediction and recommendations
                    with st.spinner("Generating AI predictions..."):
                        ...
                        # Add disclaimer
                        st.info("""
                        Note: Predictions are based on AI analysis and historical trends. 
                        Actual market prices may vary due to unforeseen factors.
                        """)
    ...
    with st.form("pest_control_form"):
        ...
        with col1:
            crop_type = st.selectbox(
                ...
                index=0,
                help="Select the crop you need pest control advice for"
            )
        with col2:
            growth_stage = st.selectbox(
                ...
                index=1,
                help="Select current growth stage of your crop"
            )
        
        # Problem description
        symptoms = st.text_area(
            "Describe Symptoms or Upload Images*",
            placeholder="e.g., Yellow spots on leaves, stunted growth, holes in leaves...",
            help="Describe visible symptoms or upload images of affected plants"
        )
        
        # Weather conditions
        weather_conditions = st.selectbox(
            "Recent Weather Conditions",
            ["Dry", "Humid", "Rainy", "Alternating Wet/Dry", "Extreme Heat"],
            index=1,
            help="Select prevailing weather conditions"
        )
        ...
        if analyze_pest:
            with st.spinner("Analyzing crop health..."):
                try:
                    # AI-generated pest analysis
                    model = genai.GenerativeModel('gemini-1.5-flash')
                    
                    # Generate pest identification
    ...
    # Image upload section (would be connected to actual image analysis in production)
    ...
    with st.form("soil_analysis_form"):
        ...
        with col2:
            crop_planned = st.text_input(
                "Planted/Planned Crop",
        ...
        with col1:
            ph_level = st.slider(
                ...
                value=7.0,
                step=0.1,
                help="Soil pH value (ideal: 6.0-7.5 for most crops)"
            )
        with col2:
        ...
        with col3:
            micronutrients = st.selectbox(
                "Micronutrient Status",
                ["Deficient", "Marginal", "Adequate", "Sufficient"],
                index=2,
                help="Overall micronutrient availability"
            )
        
        # Additional observations
        observations = st.text_area(
            "Additional Observations",
            placeholder="e.g., Hard crust formation, poor drainage, visible salt deposits...",
            help="Any other noticeable soil characteristics"
        )
            ...
            with st.spinner("Analyzing soil health..."):
                try:
                    ...
                    with col1:
                        st.metric("pH Status", 
                                f"{ph_level}",
                                help=f"Ideal range for {crop_planned}: 6.0-7.5" if crop_planned else "Ideal range: 6.0-7.5")
                    with col2:
                        st.metric("Organic Carbon", 
                                f"{organic_carbon}%",
                                "Aim for >1%" if organic_carbon < 1 else "Good level")
                    with col3:
                        st.metric("NPK Balance", 
    ...
    with st.form("fertilizer_form"):
        ...
        with col2:
            growth_stage = st.selectbox(
                ...
                ["Pre-planting", "Seedling", "Vegetative", 
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            nitrogen = st.select_slider(
                "Nitrogen Level",
                options=["Very Low", "Low", "Medium", "High", "Very High"],
                value="Medium"
            )
        with col2:
            phosphorus = st.select_slider(
                "Phosphorus Level",
                options=["Very Low", "Low", "Medium", "High", "Very High"],
                value="Medium"
            )
        with col3:
            potassium = st.select_slider(
                "Potassium Level",
                options=["Very Low", "Low", "Medium", "High", "Very High"],
                value="Medium"
            ) 
        ...
        with col2:
            organic_matter = st.select_slider(
                "Organic Matter",
        ...
        irrigation_type = st.selectbox(
            "Irrigation Method",
            ["Flood", "Drip", "Sprinkler", "Rainfed", "Other"],
            index=1
        )
                ...
                try:
                    ...
                    with col2:
                        selected_fertilizer = st.selectbox(
                            ...
                            ["Urea", "DAP", "MOP", "NPK Complex", "Organic Manure"],
                            index=0
                        )         
    ...
    with st.expander("📖 Fertilizer Application Tips"):
        st.markdown(
            ...
            <div class="content-box">
                ...
                <ul>
                    <li><strong>Timing:</strong> Apply fertilizers when soil is moist but not waterlogged</li>
                    <li><strong>Method:</strong> Band placement near roots is better than broadcasting</li>
                    <li><strong>Safety:</strong> Wear gloves and mask when handling chemical fertilizers</li>
    ...
    with st.form("rotation_form"):
        ...
        with col1:
            current_crop = st.selectbox(
                "Current/Most Recent Crop",
                ["Rice", "Wheat", "Cotton", "Sugarcane", "Maize", 
                 "Pulses", "Oilseeds", "Vegetables", "Fallow", "Other"],
                index=0
            )
        with col2:
            seasons_grown = st.number_input(
                "Seasons Grown Continuously",
                min_value=1,
                max_value=10,
                value=1,
                help="Number of consecutive seasons this crop was grown"
            )
        # Soil health parameters
        st.markdown('<p class="section-header">Soil Health Indicators</p>', unsafe_allow_html=True)  
        col1, col2, col3 = st.columns(3)
        with col1:
            pest_problems = st.multiselect(
                "Recent Pest/Disease Issues",
                ["Nematodes", "Fungal Diseases", "Bacterial Wilt", 
        ...
        with col3:
            organic_matter = st.select_slider(
                ...
                options=["Very Low (<0.5%)", "Low (0.5-1%)", "Medium (1-2%)", 
        ...
        with col1:
            irrigation_type = st.selectbox(
                "Irrigation Method",
                ["Rainfed", "Flood", "Drip", "Sprinkler", "Mixed"],
                index=0
            )
        with col2:
            farm_size = st.selectbox(
                "Farm Size Category",
                ["Small (<2 ha)", "Medium (2-5 ha)", "Large (>5 ha)"],
                index=1
            )
        # Rotation preferences
        rotation_preference = st.radio(
            "Rotation Priority",
            ["Soil Health Improvement", "Disease Break", "Income Stability", "Labor Reduction"],
            index=0
        )
        ...
        if generate_plan:
            with st.spinner("Creating optimized rotation plan..."):
                try:
                    ...
                    # Sample calendar data (would be dynamic in full implementation)
                    calendar_data = {
    ...
    # Check if farm profile exists in session state
